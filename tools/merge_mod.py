...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        stage = Path(tmp)
        archives[0].extractall(
            stage,
            members=[n for n in entry_names(archives[0]) if not n.startswith("lib/")]
        )
        for archive in archives:
            archive.extractall(
                stage,
                members=[n for n in entry_names(archive) if n.startswith("lib/")]
            )

        mod_libs = []
        for platform in sorted(platform_sources):
            libs = [
                p for p in (stage / "lib" / platform / n for n in MOD_LIB_NAMES)
                if p.is_file()
            ]
            if len(libs) != 1:
